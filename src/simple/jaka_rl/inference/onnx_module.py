"""ONNX inference wrapper for the Jaka MF policy.

Ported from sim2real-jaka ``rl_policy/inference/onnx_module.py``. Handles
input/output name normalization (``_orig`` / ``next_*``), dynamic leading
batch dims, and loading of an optional JSON meta file describing the in/out
keys.
"""
...
class ONNXModule:
    ...
    def _get_input_value(
        ...
        raise KeyError(f'Missing ONNX input for binding "{ort_input_name}" (expected key "{key}")')
    ...
    def __call__(
        ...
        outputs = {k: _squeeze_output_value(v) for k, v in zip(self.out_keys, outputs)}
        return outputs
