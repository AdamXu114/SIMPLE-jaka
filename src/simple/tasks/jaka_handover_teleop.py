...
Jaka Khan Mini teleop task — hand over the object to the container.
Mirrors the G1 handover teleop task with the robot swapped to "jaka".
"""
...
@TaskRegistry.register("jaka_handover_teleop")
class JakaHandoverTeleop(Task):
    uid: str = "jaka_handover_teleop"
    label: str = "Jaka Handover Teleop"
    description: str = (
        "A task where the Jaka robot must hand over the object to the container."
    )
    ...
    dr_cfgs: dict[str, Any] = dict(
        ...
        spatial=SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1.3, 0, 0], high=[-1.32, 0, 0]),
            target_region=Box(low=[-0.31, 0.02], high=[-0.27, -0.04]),
            container_region=Box(low=[-0.3, 0.35], high=[-0.25, 0.45]),
        ...
        scene=TabletopSceneDRCfg(
            table_position=Box(low=[0.3, 0.25], high=[0.3, 0.25]),
