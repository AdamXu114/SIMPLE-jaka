...
Standalone Jaka teleop task: move and pick up the cheezit (sim-to-real, two tables).
"""
...
@TaskRegistry.register("jaka_mobile_pick_and_place_cheezit_teleop")
class JakaMobilePickAndPlaceCheezitTeleop(Task):
    uid: str = "jaka_mobile_pick_and_place_cheezit_teleop"
    label: str = "Jaka MobilePickAndPlaceCheezit Teleop"
    description: str = (
        "A task where the Jaka robot must move and pick up the cheezit in the "
        "sim-to-real two-table setup."
    )
    ...
    dr_cfgs = dict(
        language=LanguageDRCfg(instructions=["move and pick up the cheezit"]),
