...
Jaka Khan Mini tabletop pick teleop task — grasp a fixed object on the tabletop.
Mirrors the Jaka open-trash-can teleop spatial layout but WITHOUT the trash can
(no articulated object), and uses a fixed, manually-chosen target object.
"""
...
@TaskRegistry.register("jaka_tabletop_pick_teleop")
class JakaTabletopPickTeleop(Task):
    """Jaka tabletop pick teleop — grasp a fixed object on the tabletop (no trash can)."""

    uid: str = "jaka_tabletop_pick_teleop"
    label: str = "Jaka Tabletop Pick Teleop"
    description: str = (
        "A task where the Jaka robot must pick up the fixed object placed on the tabletop."
    )
    ...
    dr_cfgs: dict[str, Any] = dict(
        ...
        spatial=SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1.4, 0, 0.0], high=[-1.5, 0.0, 0.0]),
            # robot_orientation_region=Box(low=[0.7071, 0, 0, 0.7071], high=[0.7071, 0, 0, 0.7071]),  # ← 新增
            # Place the object on the NEAR / robot-facing half of the table
