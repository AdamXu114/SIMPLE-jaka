...
@TaskRegistry.register("jaka_bend_pick_sim_to_real_teleop")
class JakaBendPickSimToRealTeleop(Task):
    uid: str = "jaka_bend_pick_sim_to_real_teleop"
    label: str = "Jaka BendPick SimToReal Teleop"
    description: str = (
        "A task where the Jaka robot must bend down and pick up the object, "
        "configured for sim-to-real transfer."
    )
    ...
    dr_cfgs: dict[str, Any] = dict(
        language=LanguageDRCfg(instructions=["pick up the object"]),
        target=TargetDRCfg(asset_id="graspnet1b:10"),
        distractors=DistractorDRCfg(
            res_id="graspnet1b", number_of_distractors=3, allow_duplicates=False, exclude=["10"],
        ),
        ...
        scene=TabletopSceneDRCfg(
            ...
            scene_manager="hssd", randomize_scene_pose=False,
        ),
