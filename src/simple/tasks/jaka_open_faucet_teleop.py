...
Jaka Khan Mini teleop task — open the faucet.
Mirrors the G1 open faucet teleop task with the robot swapped to "jaka".
"""
...
@TaskRegistry.register("jaka_open_faucet_teleop")
class JakaOpenFaucetTeleop(Task):
    uid: str = "jaka_open_faucet_teleop"
    label: str = "Jaka OpenFaucet Teleop"
    description: str = (
        "A task where the Jaka robot must open the faucet."
    )
    ...
    dr_cfgs: dict[str, Any] = dict(
        language=LanguageDRCfg(instructions=["open the faucet"]),
        ...
        articulated=ArticulatedObjectDrCfg(asset_id="articulated:4"),
        spatial=SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1.7, 0, 0.0], high=[-1.8, 0.0, 0.0]),
        ...
        scene=TabletopSceneDRCfg(
            ...
            room_choices=["hssd:scene2"], scene_manager="hssd",
        ),
