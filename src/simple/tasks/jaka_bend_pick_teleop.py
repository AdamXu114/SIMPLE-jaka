...
@TaskRegistry.register("jaka_bend_pick_teleop")
class JakaBendPickTeleop(Task):
    uid: str = "jaka_bend_pick_teleop"
    label: str = "Jaka BendPick Teleop"
    description: str = (
        "A task where the Jaka robot must bend down and pick up the object."
    )
    ...
    dr_cfgs: dict[str, Any] = dict(
        language=LanguageDRCfg(instructions=["bend down and pick up the object"]),
        ...
        spatial=SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1.2, -0.0, 0.], high=[-1.26, 0.0, 0.]),
            target_region=Box(low=[-0.12, -0.08], high=[-0.17, -0.04]),
