...
Standalone Jaka teleop task: pick up the apple and place it in the basket.
"""
...
@TaskRegistry.register("jaka_pick_and_place_apple_teleop")
class JakaPickAndPlaceAppleTeleop(Task):
    uid: str = "jaka_pick_and_place_apple_teleop"
    label: str = "Jaka PickAndPlaceApple Teleop"
    description: str = (
        "A task where the Jaka robot must pick up the apple and place it in the basket "
        "in the sim-to-real table setup."
    )
    ...
    dr_cfgs = dict(
        language=LanguageDRCfg(instructions=["pick up the apple and place it in the basket"]),
        ...
        spatial=SpatialDRCfg(
            ...
            container_region=Box(low=[-0.32, 0.05], high=[-0.28, 0.10]),
