...
Jaka Khan Mini teleop task — close the door.
Mirrors the G1 close door teleop task with the robot swapped to "jaka".
"""
...
@TaskRegistry.register("jaka_close_door_teleop")
class JakaCloseDoorTeleop(Task):
    uid: str = "jaka_close_door_teleop"
    label: str = "Jaka CloseDoor Teleop"
    description: str = (
        "A task where the Jaka robot must close the door."
    )
    ...
    dr_cfgs: dict[str, Any] = dict(
        language=LanguageDRCfg(instructions=["close the door"]),
        ...
        articulated=ArticulatedObjectDrCfg(asset_id="articulated:3"),
        spatial=SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1.8, 1.2, 0.0], high=[-1.9, 0.8, 0.0]),
