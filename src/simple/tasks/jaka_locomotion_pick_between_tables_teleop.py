...
Standalone Jaka teleop task: move between tables and pick up the object.
"""
...
@TaskRegistry.register("jaka_locomotion_pick_between_tables_teleop")
class JakaLocomotionPickBetweenTablesTeleop(Task):
    uid: str = "jaka_locomotion_pick_between_tables_teleop"
    label: str = "Jaka LocomotionPickBetweenTables Teleop"
    description: str = (
        "A task where the Jaka robot must move between tables and pick up the object."
    )
    ...
    dr_cfgs = dict(
        language=LanguageDRCfg(instructions=["move between tables and pick up the object"]),
        target=TargetDRCfg(asset_id="graspnet1b:10"),
        container=TargetDRCfg(asset_id="objects:0"),
        distractors=DistractorDRCfg(
            res_id="graspnet1b", number_of_distractors=4, allow_duplicates=False, exclude=["10"],
        ),
        spatial=SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1.2, 0, 0], high=[-1.22, 0, 0]),
            target_region=Box(low=[-0.13, -0.04], high=[-0.11, 0.04]),
            distractors_region=[
            ...
            container_region=Box(low=[-2.7, 0.2], high=[-2.69, 0.3]),
        ...
        scene=TabletopSceneDRCfg(
            ...
            room_choices=["hssd:scene6"], scene_manager="hssd",
        ),
