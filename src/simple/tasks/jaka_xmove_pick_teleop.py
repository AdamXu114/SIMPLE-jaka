...
Jaka Khan Mini teleop task — standalone implementation (mirrors the G1
standalone teleop tasks with only the robot swapped to "jaka").
The task uses a ZMQ bridge for action/observation (sim2real-jaka protocol),
not SIMPLE's native SONIC controller.
"""
...
@TaskRegistry.register("jaka_xmove_pick_teleop")
class JakaXMovePickTeleop(Task):
    uid: str = "jaka_xmove_pick_teleop"
    label: str = "Jaka XMove Pick Teleop"
    description: str = (
        "A task where the Jaka robot must move forward and pick up the object."
    )
    ...
    dr_cfgs: dict[str, Any] = dict(
        language=LanguageDRCfg(instructions=["move forward and pick up the object"]),
        target=TargetDRCfg(asset_id="graspnet1b:10"),
        distractors=DistractorDRCfg(res_id="graspnet1b", number_of_distractors=1,
                                     allow_duplicates=False, exclude=["10"]),
        spatial=SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1.5, -0.0, 0.0], high=[-1.6, 0.0, 0.0]),
            target_region=Box(low=[-0.1, -0.08], high=[-0.05, -0.04]),
            distractors_region=Box(low=[-0.1, -0.3], high=[0.5, 0.3]),
        ...
        scene=TabletopSceneDRCfg(
            table_position=Box(low=[0.5, 0], high=[0.5, 0]),
            table_height=Box(low=0.45, high=0.58),
    ...
    def reset(
        ...
        if lang_dr is not None:
            self._instruction = lang_dr(split).format(
                self._target.asset.name if self._target is not None else "object"
            )
        else:
