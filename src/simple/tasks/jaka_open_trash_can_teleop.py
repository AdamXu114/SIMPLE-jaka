...
Jaka Khan Mini teleop task — open the trash can.
Mirrors the G1 open trash can teleop task with the robot swapped to "jaka".
"""
...
@TaskRegistry.register("jaka_open_trash_can_teleop")
class JakaOpenTrashCanTeleop(Task):
    uid: str = "jaka_open_trash_can_teleop"
    label: str = "Jaka OpenTrashCan Teleop"
    description: str = (
        "A task where the Jaka robot must open the trash can."
    )
    ...
    dr_cfgs: dict[str, Any] = dict(
        language=LanguageDRCfg(instructions=["open the trash can"]),
        ...
        articulated=ArticulatedObjectDrCfg(asset_id="articulated:5"),
        ...
        scene=TabletopSceneDRCfg(
            ...
            room_choices=["hssd:scene4"], scene_manager="hssd",
        ),
