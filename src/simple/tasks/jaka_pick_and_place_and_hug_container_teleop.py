...
Standalone Jaka teleop task: pick up the container and put it on the table.
"""
...
@TaskRegistry.register("jaka_pick_and_place_and_hug_container_teleop")
class JakaPickAndPlaceAndHugContainerTeleop(Task):
    uid: str = "jaka_pick_and_place_and_hug_container_teleop"
    label: str = "Jaka PickAndPlaceAndHugContainer Teleop"
    description: str = (
        "A task where the Jaka robot must pick up the container and put it on the table."
    )
    ...
    dr_cfgs = dict(
        language=LanguageDRCfg(instructions=["pick up the container and put it on the table"]),
        target=TargetDRCfg(asset_id="graspnet1b:10"),
        container=TargetDRCfg(asset_id="objects:0"),
        distractors=DistractorDRCfg(
            res_id="graspnet1b", number_of_distractors=1, allow_duplicates=False, exclude=["10"],
        ),
        spatial=SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.5, 1., 0], high=[-0.52, 1., 0]),
            target_region=Box(low=[0.27, 1.25], high=[0.32, 1.3]),
            distractors_region=[Box(low=[0.28, 0.73], high=[0.3, 0.70])],
            target_stable_indices=[0], target_rotate_z=Box(low=-0.15, high=0.15),
            container_region=Box(low=[0.3, 1.5], high=[0.35, 1.55]),
        ...
        scene=TabletopSceneDRCfg(
            ...
            room_choices=["hssd:scene14"], scene_manager="hssd",
        ),
