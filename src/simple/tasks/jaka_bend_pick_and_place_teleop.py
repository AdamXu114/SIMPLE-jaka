...
@TaskRegistry.register("jaka_bend_pick_and_place_teleop")
class JakaBendPickAndPlaceTeleop(Task):
    uid: str = "jaka_bend_pick_and_place_teleop"
    label: str = "Jaka BendPickAndPlace Teleop"
    description: str = (
        "A task where the Jaka robot must bend down, pick up and place the "
        "object."
    )
    ...
    dr_cfgs: dict[str, Any] = dict(
        language=LanguageDRCfg(instructions=["bend down, pick up and place the object"]),
        target=TargetDRCfg(asset_id="graspnet1b:10"),
        container=TargetDRCfg(asset_id="objects:0"),
