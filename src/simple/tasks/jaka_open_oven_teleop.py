...
Jaka Khan Mini teleop task — open the oven.
Mirrors the G1 open oven teleop task with the robot swapped to "jaka".
"""
...
@TaskRegistry.register("jaka_open_oven_teleop")
class JakaOpenOvenTeleop(Task):
    uid: str = "jaka_open_oven_teleop"
    label: str = "Jaka OpenOven Teleop"
    description: str = (
        "A task where the Jaka robot must open the oven."
    )
    ...
    dr_cfgs: dict[str, Any] = dict(
        language=LanguageDRCfg(instructions=["open the oven"]),
        ...
        articulated=ArticulatedObjectDrCfg(asset_id="articulated:2"),
        ...
        scene=TabletopSceneDRCfg(
            ...
            room_choices=["hssd:scene1"], scene_manager="hssd",
        ),
